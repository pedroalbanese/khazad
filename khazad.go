...
import (
    ...
    "github.com/pedroalbanese/khazad/internal/subtle"
)
...
func (k KeySizeError) Error() string {
    return fmt.Sprintf("khazad: invalid key size %d", int(k))
}
...
func (this *khazadCipher) Encrypt(dst, src []byte) {
    if len(src) < BlockSize {
        panic("khazad: input not full block")
    }

    if len(dst) < BlockSize {
        panic("khazad: output not full block")
    }

    if subtle.InexactOverlap(dst[:BlockSize], src[:BlockSize]) {
        panic("khazad: invalid buffer overlap")
    }
